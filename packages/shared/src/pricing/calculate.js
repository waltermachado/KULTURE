...
export function calculateFinalPrice({ product, exchangeRate, rules = DEFAULT_PRICING_RULES }) {
  ...
  // acréscimos por regra (ex.: LeBron 23 +R$300, ou +10% num tipo) — fora da base da comissão, dentro do arredondamento
  const extraRate = Number(resolved.extraRate) || 0;
  const extraRateBrl = (commissionBase + commissionBrl) * extraRate;
  const extraFixedBrl = Number(resolved.extraFixedBrl) || 0;
  const rawFinalBrl = commissionBase + commissionBrl + extraRateBrl + extraFixedBrl;
  const finalPriceBrl =
  ...
  return {
    ...
    costs: {
      ...
      extraRateBrl: roundCurrency(extraRateBrl),
