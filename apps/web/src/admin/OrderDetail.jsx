    ...
    <>
      <header className="adm-head">
        <div>
          ...
          <div className="sub">criado {fmtDateTime(order.createdAt)}{order.paidAt ? ` · pago ${fmtDateTime(order.paidAt)}` : ""}{order.shippedAt ? ` · enviado ${fmtDateTime(order.shippedAt)}` : ""}{order.deliveredAt ? ` · entregue ${fmtDateTime(order.deliveredAt)}` : ""}</div>
        </div>
      ...
      <div className="adm-grid3">
        <div>
          ...
          <section className="adm-card">
            <h3>Pagamento</h3>
            <dl className="kv">
              <dt>Gateway</dt><dd>{order.paymentProvider}</dd>
              <dt>Forma</dt><dd>{METHOD_LABELS[order.paymentMethod] || order.paymentMethod || "—"}{order.installments > 1 ? ` · ${order.installments}x` : ""}</dd>
              <dt>NSU</dt><dd><span className="mono">{order.transactionNsu || "—"}</span></dd>
              <dt>Slug/Link</dt><dd><span className="mono">{order.infinitepaySlug || "—"}</span></dd>
            ...
            {paidLike && (
              <div className="adm-toolbar" style={{ marginTop: 14, marginBottom: 0 }}>
                <button className="btn sm" disabled={busy} onClick={() => resend("paid")}>Reenviar e-mail de confirmação</button>
